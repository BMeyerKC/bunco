...
runner.test('allTablesSubmitted returns false when ghost table pre-submitted but real table still playing', (t) => {
  const tables = {
    1: { submitted: false, liveUs: 5, liveThem: 3 },
    2: { submitted: true,  usScore: 0, themScore: 0 },
  };
  t.assertEqual(allTablesSubmitted(tables, 2), false);
});

runner.test('allTablesSubmitted returns true when real and pre-submitted ghost tables are all done', (t) => {
  const tables = {
